...
#setwd ("D:/Vbox_Shared/DC_gbs/metagenomes")
for (i in c(10)) {
 # metag <- read.table("genus.txt", header=T, sep=",", check.names=FALSE, fill=TRUE)
  ...
  plot <- ggplot(corr, aes(x=Var1, y=Var2, fill=coeff, size= pvalue)) +
    geom_point(aes(size=-pvalue), shape=21) + scale_fill_gradient2(low="red", mid="white", high="cornflowerblue") +
        theme_bw() + coord_equal() + scale_size(guide = 'none') +
    labs(x="",y="",fill="Correlation\nCoeficient",size="p-value") +
    theme(axis.text.x=element_text(size=15, angle=45, vjust=1, hjust=1, margin=margin(0,0,0,0)),
          axis.text.y=element_text(size=15, margin=margin(0,0,0,0)), panel.grid.major=element_line(colour = "grey95"),
          legend.title=element_text(size=15), legend.text=element_text(size=10),legend.key.size = unit(0.3, "in"),
          plot.title = element_text(size=12)) +
    labs(title= "Genus-Level Correlogram")
  ggsave(filename=paste("Metagenome",i,"_genus_corr_spearman.tiff",sep=""), plot=plot, width=10, height=10, dpi=600, compression = "lzw", limitsize = FALSE)
}
...
for (i in c(5)) {
  ...
  plot <- ggplot(corr, aes(x=Var1, y=Var2, fill=coeff, size= ppvalue)) +
    ...
    theme_bw() + coord_equal() + scale_size(guide = 'none') +
    labs(x="",y="",fill="Correlation\nCoeficient",size="p-value") +
    theme(axis.text.x=element_text(size=9.0, angle=45, vjust=1, hjust=1, margin=margin(0,0,0,0)),
          axis.text.y=element_text(size=9.0, margin=margin(0,0,0,0)), panel.grid.major=element_line(colour = "grey95"),
          legend.title=element_text(size=8), legend.text=element_text(size=8),legend.key.size = unit(0.3, "in"),
          plot.title = element_text(size=7.5)) +
    labs(title= "Species-Level Correlogram")
  ggsave(filename=paste("Metagenome",i,"_species_corr_spearman.tiff",sep=""), plot=plot, width=6.5, height=6.5, dpi=600, compression = "lzw", limitsize = FALSE)
}
