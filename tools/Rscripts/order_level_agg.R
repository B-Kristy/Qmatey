...
order_mean  <- read.table(args[1], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
order_uniq <- read.table(args[2], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
order_quant <- read.table (args[3], header = T, sep="\t", check.names=FALSE, fill=TRUE)
