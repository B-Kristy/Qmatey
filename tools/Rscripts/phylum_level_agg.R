...
phylum_mean  <- read.table(args[1], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
phylum_uniq <- read.table(args[2], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
phylum_quant <- read.table (args[3], header = T, sep="\t", check.names=FALSE, fill=TRUE)
