...
species_mean  <- read.table(args[1], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
write.table(metag, file="species_taxainfo_mean.txt", sep="\t", row.names = FALSE)

species_uniq <- read.table(args[2], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
write.table(metag_uniq, file="species_taxainfo_unique_sequences.txt", sep="\t", row.names = FALSE)

species_quant <- read.table (args[3], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
write.table(metag_uniq, file="species_taxainfo_quantification_accuracy.txt", sep="\t", row.names = FALSE)
