...
class_mean  <- read.table(args[1], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
class_uniq <- read.table(args[2], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
class_quant <- read.table (args[3], header = T, sep="\t", check.names=FALSE, fill=TRUE)
