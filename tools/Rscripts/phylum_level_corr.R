...
phylum <- read.table(args[1], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
for (i in c(percent)) {
  ...
  metag_corr <- cor(metag, method = c("spearman")); axis_density <- (10*nrow(phylum)/nrow(metag_corr))
  metag_corr <- melt(metag_corr)
  metag_corr <- as.data.frame(metag_corr)
  metag_pmat <- cor_pmat(metag, method = c("spearman"), exact=FALSE)
  metag_pmat <- melt(metag_pmat)
  metag_pmat <- as.data.frame(metag_pmat)
  ...
  plot <- ggplot(corr, aes(x=Var1, y=Var2, fill=coeff, size= pvalue)) +
    ...
    labs(title= "Phylum-Level Correlogram")
  ggsave(filename=paste("Metagenome",i,"_phylum_corr_spearman.tiff",sep=""), plot=plot, width=35, height=35, dpi=600, compression = "lzw", limitsize = FALSE)
}
