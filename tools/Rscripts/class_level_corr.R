...
class <- read.table(args[1], header = T, sep="\t", check.names=FALSE, fill=TRUE)
...
for (i in c(percent)) {
  ...
  metag_corr <- cor(metag, method = c("spearman")); axis_density <- (10*nrow(class)/nrow(metag_corr))
  ...
  plot <- ggplot(corr, aes(x=Var1, y=Var2, fill=coeff, size= pvalue)) +
    ...
    labs(title= "Class-Level Correlogram")
  ggsave(filename=paste("Metagenome",i,"_class_corr_spearman.tiff",sep=""), plot=plot, width=35, height=35, dpi=600, compression = "lzw", limitsize = FALSE)
}
